...
class SentCNN(object):
    ...
    def __init__(self, 
        ...
        with tf.name_scope("cosine_layer"):
            ...
            self.join_layer = tf.concat(axis=2, values=[self.h_dropped_r, tf.expand_dims(self.cosine, axis=-1)])
            print("DEBUG: join_layer after cosine stack -> %s" % self.join_layer)

            tile_dropped_u = tf.tile(self.h_dropped_u, [1, num_classes, 1])

            self.join_layer = tf.concat(axis=2, values=[self.join_layer, tile_dropped_u])
            print("DEBUG: join_layer after tile_dropped_u stack -> %s" % self.join_layer)
        
            self.fc_final = tf.contrib.layers.fully_connected(inputs=self.join_layer,
                                                                    num_outputs=1,
            ...
            self.fc_final = tf.squeeze(self.fc_final)
            print("DEBUG: fc_final -> %s" % self.fc_final)
            self.predictions = tf.argmax(self.fc_final, 1, name="predictions")
            print("DEBUG: predictions -> %s" % self.predictions)
            
        # softmax regression - loss and prediction
        with tf.name_scope("loss"):
            losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=100*self.fc_final, labels=self.input_y)
